function [  ] = namingFR(patientID,nevFile)
% NAMINGFR examining firing rates for the naming task. 
%   namingFR(patientID,nevFile) plots firing rate for neurons recorded 
...
for aSS = 1:length(data)
    ...
    for ch = 1:size(data(aSS).channel,2)
        for un = 1:size(data(aSS).channel(ch).unit,2)
            ...
            display('plotting raster... If this takes forever, tell Elliot')
            ...
            for tt = 1:nTrials
                ...
                % plotting rasters (in the least efficient way possible)
